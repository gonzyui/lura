...
function scheduleReconnect() {
	...
	container.logger.warn(`[Supabase] Reconnecting guild_settings channel in ${delay}ms (attempt ${reconnectAttempts})`);
...
function subscribeGuildSettings() {
	...
	guildSettingsChannel = supabase
		...
		.on<GuildSettingsRow>('postgres_changes', { event: '*', schema: 'public', table: 'guild_settings' }, async (payload) => {
			const guildId =
			...
			container.logger.info(`[Supabase] guild_settings ${payload.eventType} for ${guildId} → cache invalidated`);
		})
		.subscribe((status, err) => {
			switch (status) {
				case REALTIME_SUBSCRIBE_STATES.SUBSCRIBED:
					reconnectAttempts = 0;
					container.logger.info('[Supabase] guild_settings channel subscribed successfully.');
					break;
